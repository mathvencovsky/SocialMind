...
-- Create a secure function for authorized token access only
CREATE OR REPLACE FUNCTION public.get_social_account_secure(account_id uuid)
RETURNS TABLE(
  id uuid,
  platform text,
  account_id text,
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE social_accounts.id = get_social_account_secure.account_id 
  ...
  WHERE sa.id = get_social_account_secure.account_id 
...
USING (
  ...
  EXISTS (
    ...
    AND profiles.bio ILIKE '%admin%' -- Simple admin check, can be enhanced
  )
);
